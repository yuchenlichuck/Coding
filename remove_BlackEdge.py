    ...
    for s in pathdir:
        ...
        if os.path.isfile(newdir):
            if os.path.splitext(newdir)[1]==".jpg":
                    ...
                    print("裁剪数量:",i)
'''
source_path="E:\\work\\sanyuan\\image_image-processing-master\\image_image-processing-master\\image2\\test\\DR\\"                                    #图片来源路径
#save_path="E:\\work\\sanyuan\\image_image-processing-master\\image_image-processing-master\\image\\test\\DRI\\"                                     #图片修改后的保存路径
...
for i in range(len(file_names)):
    change_size(source_path + file_names[i])        #得到文件名
   # cv2.imwrite(save_path+file_names[i],x)
    #print("裁剪：",file_names[i])
    print("裁剪数量:",i)
    while(i==100):
